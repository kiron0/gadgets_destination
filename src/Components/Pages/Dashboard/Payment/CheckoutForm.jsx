...
const CheckoutForm = ({ singleOrder, refetch }) => {
  ...
  useEffect(() => {
    fetch(
      `https://gadgets-destination.herokuapp.com/payment/create-payment-intent`,
      {
        method: "POST",
        headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    if (intentError) {
      ...
      if (paymentIntent?.status === "succeeded") {
        ...
        fetch(
          `https://gadgets-destination.herokuapp.com/booking?id=${singleOrder?._id}`,
          {
            method: "POST",
            headers: {
              authorization: `Bearer ${localStorage.getItem("accessToken")}`,
          ...
          .then((result) => {
            if (result?.insertedId) {
              ...
              fetch(
                `https://gadgets-destination.herokuapp.com/orders/paid/${singleOrder?._id}`,
                {
                  method: "PATCH",
                  headers: {
                    "content-type": "application/json",
