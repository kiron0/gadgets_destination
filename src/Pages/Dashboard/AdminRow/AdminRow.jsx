...
const AdminRow = ({ uid, serialize, email, role, refetch, _id }) => {
  ...
  const handleMakeAdmin = async (id) => {
    await fetch(
      `https://gadgets-destination.herokuapp.com/users/admin?uid=${id}&&currentUserId=${auth?.currentUser?.uid}`,
      {
        method: "PATCH",
        headers: {
  ...
  const handleDeleteUser = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://gadgets-destination.herokuapp.com/users?uid=${auth?.currentUser?.uid}&&deleteId=${id}`,
          {
            method: "DELETE",
            headers: {
              authorization: `Bearer ${localStorage.getItem("accessToken")}`,
